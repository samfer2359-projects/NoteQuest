...
def get_concepts(user_id):
    """Fetch all user-uploaded concepts"""
    conn = connect_db()
    cursor = conn.cursor()
...
def store_question(question_data, level):
    """Store AI-generated question safely in DB"""
    ...
    """, (
        ...
        json.dumps(question_data["options"]),  # Safe JSON storage
...
def generate_llm_question(concept, level):
    """Generate a creative AI question based on level"""
    if level <= 3:
        diff_text = "Easy"
...
def fetch_question(level, user_id):
    """Fetch an unserved question or generate a new AI question"""
    conn = connect_db()
    cursor = conn.cursor()

    # 1️⃣ Fetch existing unserved question
    cursor.execute("""
    ...
    if row:
        ...
        cursor.execute("""
            ...
            FROM Questions
            WHERE riddle_text=%s AND topic=%s
