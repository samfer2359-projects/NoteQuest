...
class Enemy {
    constructor(x, y, speed) {
        ...
        this.width = 101; this.height = 171;
...
function onTreasureCollected() {
    ...
    fetch(`/generate-question?level=${level}&user_id=${window.USER_ID}`)
        .then(r => r.json())
        .then(data => {
...
function submitAnswer(button, selected) {
    ...
    .then(r => r.json())
    .then(res => handleAnswerResponse(button, selected, res));
}
...
function handleAnswerResponse(btn, selected, res) {
    ...
    if (res.correct) {
        ...
        if (res.level_up && level < MAX_LEVEL) {
            showGameAlert("🎉 Level Up!", "success");
            scaleEnemies();
        } else {
            showGameAlert(`✅ +${res.points} points`, "success");
        }
